...
def normalize_advisory_severity(data):
    # Normalize advisory severity across raw advisory-db JSON and public API shapes.
    ...
    if isinstance(severity, dict):
        return str(severity.get("severity", "")).strip().upper() or "UNKNOWN"
    return "UNKNOWN"
...
def match_known_object(data, known_object):
    ...
    severity = normalize_advisory_severity(data)
    result = {
        "matched": False,
        "severity": severity,
        "matched_object": None,
