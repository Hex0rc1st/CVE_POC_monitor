...
- **wxvl 标题索引**：`https://raw.githubusercontent.com/gelusus/wxvl/main/data.json`
- **wxvl 仓库快照**：`https://codeload.github.com/gelusus/wxvl/tar.gz/refs/heads/main`
...
脚本会对中文博客、论坛和安全分析文章打分，并过滤通告型内容。

## 微信公众号 PoC 搜索

仓库内也提供了一个基于 `gelusus/wxvl` 归档的微信公众号文章检索脚本。
...
该脚本优先使用 wxvl 标题索引，只有必要时才回退到本地缓存 Markdown，并会过滤奇安信、360、微步、长亭、FreeBuf 等官方/通告型来源。

## 多源 PoC 查询

使用统一入口同时查询 GitHub、中文互联网和微信公众号文章：
